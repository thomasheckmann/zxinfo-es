...
FROM 
  authors aut 
  INNER JOIN labels dev ON aut.label_id = dev.id 
  LEFT JOIN labeltypes dlt ON dlt.id = dev.labeltype_id 
  LEFT JOIN countries ac1 ON dev.country_id = ac1.id 
  LEFT JOIN labels team ON aut.team_id = team.id 
  LEFT JOIN labeltypes tlt ON tlt.id = team.labeltype_id 
  LEFT JOIN countries tc1 ON team.country_id = tc1.id 
  LEFT JOIN roles r ON r.entry_id = aut.entry_id 
  AND aut.label_id = r.label_id 
  LEFT JOIN roletypes rt ON rt.id = r.roletype_id 
  LEFT JOIN notes n ON n.label_id = dev.id 
...
var getAuthors = function (id) {
  ...
  connection.query(
    "SELECT IF( r.roletype_id IN('A', 'T'), 'Contributor', 'Creator' ) AS type, author_seq AS authorSeq, dev.name AS name, ac1.text AS country, dlt.text AS labelType, r.roletype_id AS roleType, rt.text AS roleName, team.name AS groupName, tc1.text AS groupCountry, tlt.text AS groupType, n.text, nt.id AS notetypes FROM authors aut INNER JOIN labels dev ON aut.label_id = dev.id LEFT JOIN labeltypes dlt ON dlt.id = dev.labeltype_id LEFT JOIN countries ac1 ON dev.country_id = ac1.id LEFT JOIN labels team ON aut.team_id = team.id LEFT JOIN labeltypes tlt ON tlt.id = team.labeltype_id LEFT JOIN countries tc1 ON team.country_id = tc1.id LEFT JOIN roles r ON r.entry_id = aut.entry_id AND aut.label_id = r.label_id LEFT JOIN roletypes rt ON rt.id = r.roletype_id LEFT JOIN notes n ON n.label_id = dev.id LEFT JOIN notetypes nt ON nt.id = n.notetype_id WHERE aut.entry_id = ? ORDER BY authorSeq, roleType, name",
    [id],
