 ...
 * CHANGELOG:
 * 01.01.2021
 *  https://spectrumcomputing.co.uk/forums/viewtopic.php?p=71154#p71154
 *   - compilations, variationtypes -> contents, contenttypes
 */
"use strict";
...
var getCompilationContent = function (id) {
  ...
  var connection = db.getConnection();
  connection.query(
    "SELECT ecomp.media_side AS tape_side, ecomp.media_seq AS tape_seq, ecomp.prog_seq AS prog_seq, ecomp.alias AS alias, item.id AS id, item.title AS title, ll.name AS publisher, lc1.text AS country, lt.text AS labeltype, evart.text AS variation, machinet.text AS machinetype FROM contents ecomp LEFT JOIN entries item ON ecomp.entry_id = item.id LEFT JOIN machinetypes machinet ON item.machinetype_id = machinet.id INNER JOIN contenttypes evart ON ecomp.contenttype_id = evart.id LEFT JOIN publishers p ON p.entry_id = ecomp.entry_id LEFT JOIN labels ll ON p.label_id = ll.id LEFT JOIN labeltypes lt ON lt.id = ll.labeltype_id LEFT JOIN countries lc1 ON ll.country_id = lc1.id WHERE ( ( p.label_id IS NOT NULL AND p.release_seq = 0 ) OR( p.label_id IS NULL AND p.release_seq IS NULL ) ) AND ecomp.container_id = ? ORDER BY tape_seq, tape_side, prog_seq;",
    [id],
    function (error, results, fields) {
      if (error) {
        throw error;
