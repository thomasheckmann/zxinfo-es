 ...
 * 
 * 13.07.2023
 *  https://spectrumcomputing.co.uk/forums/viewtopic.php?p=120689#p120689
 *  - derived release date (release 0, derived e.g. from cover magazine)
 */
"use strict";
 ...
 * Get basic info
 *
 * Release year - the date in RELEASES (0) has priority over "derived" release date. For example if entry has been released as part of a cover tape or similar.
 */
var getBasicInfo = function (id) {
  var deferred = Q.defer();
  var connection = db.getConnection();
  connection.query(
    'SELECT e.title AS fulltitle, GROUP_CONCAT( DISTINCT aka.title ORDER BY aka.title ASC SEPARATOR ";;" ) AS alsoknownas, IF(COALESCE(r.release_year, r.release_month, r.release_day), r.release_year, so.date_year) AS yearofrelease, IF(COALESCE(r.release_year, r.release_month, r.release_day), r.release_month, so.date_month) AS monthofrelease, IF(COALESCE(r.release_year, r.release_month, r.release_day), r.release_day, so.date_day) AS dayofrelease, machinet.text AS machinetype, e.max_players AS numberofplayers, (SELECT GROUP_CONCAT(g.name) FROM members turn INNER JOIN tags g ON turn.tag_id = g.id and g.tagtype_id = "N" WHERE turn.entry_id = e.id) AS multiplayermode, (SELECT GROUP_CONCAT(g.name) FROM members turn INNER JOIN tags g ON turn.tag_id = g.id and g.tagtype_id = "Y" WHERE turn.entry_id = e.id) AS multiplayertype, e.genretype_id AS genretype, entryt.text AS type, e.is_xrated AS xrated, r.book_isbn AS isbn, idm.text AS messagelanguage, r.release_price AS originalprice, c.name AS cur_name, c.symbol AS cur_symbol, c.prefix AS cur_prefix, availt.text AS availability, so.publication as originalpublication, sc.score AS score, sc.votes AS votes, v.version FROM entries e LEFT JOIN zxinfo_version v ON 1=1 LEFT JOIN releases r ON r.entry_id = e.id LEFT JOIN currencies c ON r.currency_id = c.id LEFT JOIN aliases aka ON aka.entry_id = r.entry_id AND aka.release_seq = r.release_seq LEFT JOIN availabletypes availt ON e.availabletype_id = availt.id LEFT JOIN machinetypes machinet ON e.machinetype_id = machinet.id LEFT JOIN genretypes entryt ON e.genretype_id = entryt.id LEFT JOIN languages idm ON e.language_id = idm.id LEFT JOIN scores sc ON sc.entry_id = e.id LEFT JOIN search_by_origins so ON so.entry_id = e.id WHERE e.id = ? AND(r.release_seq = 0 OR r.release_seq IS NULL ) GROUP BY e.id;',
    [id],
