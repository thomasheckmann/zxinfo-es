...
var create_doc = function(id) {
    return Q.all([getBasicInfo(id),
        getPublisher(id),
        getReReleasedBy(id),
        getAuthors(id),
        getAuthoring(id),
...
var zxdb_doc = function(id) {
    create_doc(id).done(function(results) {
        var i = 0;
        var doc_array = {};
        for (; i < results.length; i++) {
...
var getAllIDs = function() {
    ...
    //connection.query('select id from entries where id < 10000 order by id asc', function(error, results, fields) {
    //connection.query('select id from entries where id >= 10000 and id < 20000 order by id asc', function(error, results, fields) {
    // connection.query('select id from entries where id >= 20000 and id < 30000 order by id asc', function(error, results, fields) {
    // connection.query('select id from entries where id >= 28000 and id < 30000 order by id asc', function(error, results, fields) {
    connection.query('select id from entries where id >= 30000 order by id asc', function(error, results, fields) {
        if (error) {
            throw new Error("Can't connect: ", err.stack);
        }
        var ids = [];
...
getAllIDs().then(function(result) {
    var i = 0;
    for (; i < result.length; i++) {
        zxdb_doc(result[i]);
    }
}).done(function(r) { console.log("### REAL END!") });
